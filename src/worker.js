...
let userToken = ''
const go = new Go()
WebAssembly.instantiateStreaming(fetch('/src/assets/brand/json.wasm'), go.importObject).then(
  (result) => {
    go.run(result.instance)
    defaultToken = getDefaultToken(true)
  },
)
...
function postApi(inData) {
  ...
  if (inData.userToken == true) myHeaders.append('Authorization', 'Bearer ' + userToken)
  else myHeaders.append('Authorization', 'Bearer ' + defaultToken)
  //
...
function fetch2Coins() {
  self.postMessage(defaultToken)
}
function fetchCoins() {
